...
Должно быть **236 passed**. Если нет — дальше не идти.

Тесты идут ДО `.env` намеренно: секретов они не требуют. Так и проверяется, что
код не завязан на окружение при импорте — на этом деплой уже спотыкался
(`API_ID` читался на уровне модуля, и тесты не собирались вовсе).
