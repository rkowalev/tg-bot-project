...
no_data — пост НЕ ОПИСЫВАЕТ вакансию: нет ни обязанностей, ни требований, ни \
стека, всё за ссылкой ("подробности в LinkedIn", "смотреть вакансию", \
"описание на GeekJob"). Зарплата и формат при этом могут быть указаны — они не \
помогают, потому что главный вопрос (тот ли язык) остаётся без ответа. Это НЕ \
medium: medium значит "язык подходит, но есть расхождение", а тут неизвестен \
сам язык, и medium соврал бы, что он совпал. И это НЕ low: low — "проверили и \
мимо", а здесь проверять было нечего.

ВАЖНО, где проходит граница. Решает НАЛИЧИЕ ОПИСАНИЯ, а не наличие языка. \
Если в посте перечислены обязанности и требования — это НЕ no_data, даже если \
язык не назван ни разу. Такое бывает у вакансий на РУЧНОЕ тестирование: язык \
там не упомянут не потому, что пост пустой, а потому что программировать не \
требуется. Это известный факт, а не неизвестность, и оценивать его надо \
обычной шкалой: ручное тестирование без автоматизации — low, пользователь идёт \
в автоматизацию.
