...
class Score(str, Enum):
    ...
    # Пост-заглушка: должность есть, а сути нет — всё за ссылкой. Отдельная
    # оценка, потому что "нечего оценивать" и "спорно" — разные вещи, а в
    # medium они выглядели одинаково. В low такое класть нельзя: low не
    # показывается вообще, а за ссылкой может быть отличная вакансия.
    NO_DATA = "no_data"


class RelevanceResult(BaseModel):
    score: Score = Field(
        description="high — подходит, стоит откликаться. medium — спорно, надо смотреть глазами. low — формально прошло правила, но по сути мимо. no_data — в посте нет содержания, судить не по чему."
    )
    reasoning: str = Field(
...
no_data — в посте нет ТЕХНИЧЕСКОЙ сути: не названы ни язык, ни стек, ни \
задачи, всё это за ссылкой ("подробности в LinkedIn", "смотреть вакансию"). \
Зарплата и формат при этом могут быть указаны — они не помогают, потому что \
главный вопрос (тот ли язык) остаётся без ответа. Это НЕ medium: medium \
значит "язык подходит, но есть расхождение", а тут неизвестен сам язык, и \
medium соврал бы, что он совпал. И это НЕ low: low — "проверили и мимо", а \
здесь проверять было нечего. Если язык или задачи названы хоть как-то — \
оценивай обычной шкалой, no_data не ставь.

Если зарплата не указана, но остальное в посте есть — это минус в сторону \
medium, но не приговор. Отсутствие ОДНОЙ только зарплаты — не повод для no_data.
